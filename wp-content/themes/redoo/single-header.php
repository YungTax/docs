         ...
         if ($categories) {
            echo '<div class="themelazer_post_categories">';
               foreach( $categories as $tag) {
                  ...
                  $title_reactions = get_term_meta($tag->term_id, "redoo_cat_reactions", true);
                  if($title_reactions){
...
<?php if(has_post_format('gallery')){ ?>  
   <div class="themelazer_single_feature">
      <div class="justified-gallery-post">
         <?php $image_gallery_val = get_post_meta( get_the_ID(), 'gallery_post_format' , true); if($image_gallery_val !== ""){
            $image_gallery_array = explode(',',$image_gallery_val);
            if(isset($image_gallery_array) && count($image_gallery_array)!= 0):
               foreach($image_gallery_array as $gimg_id):
                     $the_image = wp_get_attachment_image_src( $gimg_id, 'redoo_justify_feature' );
                     $the_caption = get_post_field('post_excerpt', $gimg_id); ?>
                     <a<?php if($the_caption) {echo ' title="'.esc_attr($the_caption).'"';}?> class="featured-thumbnail" 
                        href="<?php echo esc_url($the_image[0]); ?>">
...
<?php 
   ...
   <?php $slider_large_thumb_id = get_post_thumbnail_id(); if($slider_large_thumb_id){ ?>
      <div class="themelazer_single_feature">
         <div class="themelazer_full_post">
            <?php if(get_post_meta( $post->ID, 'quote_post_title', true )){?>
            <?php
               $slider_large_image_header = wp_get_attachment_image_src( $slider_large_thumb_id, 'redoo_justify_feature', true ); ?>
            <?php if($slider_large_thumb_id){?>
                     <div class="themelazer_item_img" style="background-image: url('<?php echo esc_attr($slider_large_image_header[0]); ?>')">
                     </div>
            <?php }else{?>
                     <div class="themelazer_item_img" style="background-image: url('<?php echo esc_url(get_template_directory_uri().'/img/feature_img/header_carousel.jpg');?>')">
                     </div>
            <?php }?>
                  <div class="single_header_wrapper">
                     <blockquote>
   ...
   <div class="themelazer_single_feature"><?php
         ...
         if($audio_embed !=""){
           echo get_post_meta( $post->ID, 'auto_post_embed', true );
            ...
            if ( has_post_thumbnail()) {
               the_post_thumbnail('redoo_justify_feature');
            }
...
<?php if ( has_post_thumbnail()) {?>
  <div class="themelazer_single_feature">
     <?php if ( has_post_thumbnail()) {the_post_thumbnail('redoo_justify_feature');}?>
  </div>
<?php }}?>
